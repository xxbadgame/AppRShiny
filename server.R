...
server <- function(input, output) {
  ...
  ####### INTEGRER LA SOLUTION D'IMAD, J'AI DEJA MIS LE FRONT AVEC LES BOX MAIS JE N'AI PAS REUSSI POUR LE BACK #####
    ...
    observeEvent(input$bouton_recherche, {
      ...
      ### KPI 7 
      velo_elec_batterie_removable = subset(VelovList$totalStands$availabilitie$electricalRemovableBatteryBikes, VelovList$number== recherche)
      
      output$VeloElecBatterieRemovable_box <- renderText ({
        velo_elec_batterie_removable
      })
